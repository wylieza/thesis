...
The target MCU software was separated into three independent processes for the purposes of this investigation.
...
The decoding process is an interrupt triggered processes which may be started by software. This process decodes the time stamps recorded by the edge detection process and returns the recovered data. The decoding process uses the state machine approach illustrated in figure \ref{fig:manchesterdecodingfsm}.
...
The maximum number of edges in a valid message occurs when all 16-bits are 1's. The resulting Manchester sequence contains 36 edges. %here
