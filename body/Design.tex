...
Even after the above two means of optimizing the goertzel algorithm, the STM32F0 was not fast enough to process every sample. %HERE
...
The result, $333.\dot{3}$ cannot be written as the product of two integer values, therefore it is chose that the pre-scalar was set to one and the clock period was set to 333. Therefore the actual sampling frequency is $144.\dot{1}4\dot{4}$ kHz. Returning to equation \ref{eqn:k_N_and_fb_fs_ratio}, we see that we must choose $f_{bin} = 36.036$kHz to preserve the ratio.
