...
% - Maybe dont include paragraph:
%It is important to reiterate at this point that the goal of this investigation is not to design a ready to play system, but rather develop modules based on a breakdown of the system and then evaluate how these modules perform.
...
%todo: explain why designs are so ellaborate... This investigation is centred on the creation and evaluation of system modules, therefore the design descriptions have been written 
...
The final module of the target subsystem is the main processor which is implemented on the UCT STM32 development board.
...
In most cases the incoming signal is a square wave with an amplitude of 4.5V which has arisen due to the gain of the IR detector module causing it to saturate. This high voltage exceeds the tolerance of the ADC, therefore a voltage divider formed by R1 and R2 prior to the filter is used to reduce the signal to an acceptable value.

The filter circuit used is a second order voltage controlled voltage source (VCVS) active low pass filter in a Chebyshev configuration. The component values where chosen in accordance to the filter design table found on page 274 of the book \textit{The art of electronics - 2nd Edition}\cite{Horowitz1995}.
