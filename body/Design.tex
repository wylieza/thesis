...
\textbf{Omega Value}\\
The value of omega cannot be set directly, rather its relationship to the target bin frequency and sampling frequency must be established so that by carefully choosing these, the value of omega can be indirectly manipulated.
...
Interrupts and global variables open the possibility of data corruption. Although the STM32F0 is inherently a serial processor, the main-loop process (responsible for, among other things, initiating transmissions) and the transmission process share some properties with parallel processes. Figure \ref{fig:parallel_process_abstraction} illustrates how the operations performed during transmission can be abstracted as two parallel processes. Therefore it is necessary to provide a mechanism to prevent one process from modifying any of the shared variables (given in list \ref{lst:transmission_status_tracking_variables}) while the other is using them.
...
The variable \textit{fire\_in\_progress} operates as a synchronization lock. As priority is given to the interrupt process, there is a loop in the implementation of the function \textit{generate\_manchester} which stalls the processor until any current transmissions complete, as indicated by this lock (see line 4, listing \ref{lst:generate_manchester_implementation}).
...
In laser tag, it is optimal to have multiple IR detectors placed in various locations on the player. It is therefore important that the target software has the capacity to process transmissions from multiple inputs simultaneously. This is achieved through the use of the input capture and output compare channels of the STM32's timer peripherals. For this investigation, the system is designed to monitor a single target, however, the process is easily expanded to capture signals from multiple targets.
