...
In laser tag it is optimal to have multiple IR detectors placed on various parts of the player. It is therefore important that the tagger software be extendable to process input from multiple input signals simultaneously. To achieve this, the input capture and output capture functions of the STM32 timer peripherals were used. For the purposes of this investigation, the system was developed to only capture a single incoming signal, however the design allows this to be easily expanded to capture multiple incoming signals.


\subsubsection{Timing Constraints}

%todo: tidy this paragraph up a tad - verify the assertions....
During transmission various factors can influence the timing between edges, due to variations in the carrier frequency or the received signal amplitude. These variations are introduced by the Goertzel filter which is sensitive to these changes. Therefore the receiver should be designed to be tolerant to small timing deviations.

The state machine approach for decoding as outlined in figure \ref{fig:manchesterdecodingfsm} was used. Because this appeach does not  incorporates large timing tolerances by design. The state machine as shown is incomplete, there is no action associated with a long delay while in either the \textit{start of 'zero'} or \textit{start of 'one'} states. In the event of this condition, an error has occured. %here
...
The timeout period was chosen to be a length of two bit-periods, this was chosen because future inclusion of hamming codes would allow for the correction of a flipped or missing bit. The timeout period of 3.556ms must be converted into a number of ticks before being used by the output compare functionality of the timer.
