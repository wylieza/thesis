...
The software design for this module is presented in the following way: %improve this sentence
...
In addition to the sampling frequency, the number of samples (N) processed by the Goertzel algorithm determines the frequency response of the filter. For decreasing N, the bandwidth of each frequency bin increases which causes the tone detector to trigger for a wider band of frequencies. For increasing N, the time taken to collect and process the samples increases. In order for the filter to be effective, the processing period must be significantly short than the modulation period.


%todo: this section is an absolute mess (some part are cut to snippets on overleaf) - come back later
\subsubsection{Filter Optimization}
\label{sec:filter_optimization_design}
The Goertzel filter is optimized in two different ways. The first speed up is achieved by configuring the timer, ADC and DMA to continuously sample the incoming signal and store the conversion results in a circular buffer. Without these peripherals, it would not have been possible to perform filtering in real-time.

The second means of optimization is performed through the manipulation of algorithm parameters to reduce the number of multiplications. By manipulating $omega$ to equal $\frac{\pi}{2}$, the algorithm constants $cosval$ and $sinval$ are equal to zero and one respectively (see listing \ref{lst:goertzel_algorithm}). This manipulation reduces instruction on line 13 to $s0 = samples(i) - s2$.

Even after these optimizations the STM32F0 is not fast enough to process every sample. This is solved by sampling twice as many samples, but only using the first N samples to calculate the DFT coefficient. This solution works because the sections of waveform that go unprocessed are much shorter than the modulation period.
%todo: replace goertzel value with DFT coefficent

\textbf{Selecting the Sampling Frequency}\\
...
Each Goertzel calculation used a total of 16 samples. This number of samples provided a good balance between the bandwidth of the DFT frequency bin and the processing delay.
...
The original RC-5 protocol specifies a period of 114mS between transmissions, however, this period is longer than necessary as shown in section \ref{sec:processing_time_experiment}. Based on the results, this period was reduced to 50mS.
