...
To determine if it is possible to reduce the dead-time between transmissions, the following experiment was performed to measure the time it takes the processor to decode Manchester encoded transmissions. In addition to determining if the dead-time may be reduced, the experiment also aims to determine if the number of edges in a given transmission has any effect on the processing time.
